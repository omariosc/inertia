...
public class DbInitializer
{
    ...
    public static void Initialize(InertiaContext context, UsersService users, IWebHostEnvironment env)
    {
        ...
        if (env.IsDevelopment())
        {
            users.CreateAccount("inertiateam+test@outlook.com", "test_password", "Le Customer", UserType.Regular, AccountRole.User).Wait();
            users.CreateAccount("inertiateam+test2@outlook.com", "test_password", "Le Staff", UserType.Regular, AccountRole.Manager).Wait();
        }
