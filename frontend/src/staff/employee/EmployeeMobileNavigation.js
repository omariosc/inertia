 ...
 * on mobile devices
 * @return {JSX.Element} Employee mobile navbar
 */
export default function EmployeeMobileNavigation() {
  ...
  const closeIcon = <CgClose className="hamburger-menu-staff" color="white"
    size="35" onClick={() => setOpen(!open)}/>;
  const [, signOut] = useAccount();
  const navigate = useNavigate();

  /**
   * Groups the different links required for the navigation bar
   * @return {JSX.Element} The group of links for mobile navbar
   */
  function Links() {
    return (
      <Nav
      ...
      >
        ...
        <Nav.Link as={Link} to="/create-guest-booking"
          className="nav-indented">
            Create Booking
        </Nav.Link>
        <Nav.Link as={Link} to="/booking-applications"
          className="nav-indented">
