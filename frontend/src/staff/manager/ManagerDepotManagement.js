 ...
 * depots available
 * @return {JSX.Element} The manager depot management page
 */
export default function ManagerDepotManagement() {
  ...
  async function editDepot(id, mode) {
    ...
    switch (mode) {
      ...
      case 2:
        ...
        if (parseFloat(newLongitude) >= 180 || parseFloat(newLongitude) <=
            -180) {
          NotificationManager.error('Enter a valid longitude value.', 'Error');
          return;
        }
    ...
    try {
      const request = await fetch(host + `api/admin/Depos/${id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
  ...
  async function createDepot() {
    ...
    if (parseFloat(createLatitude) >= 180 || parseFloat(createLatitude) <=
        -180) {
      NotificationManager.error('Enter a valid latitude value.', 'Error');
      return;
    }
    ...
    if (parseFloat(createLongitude) >= 180 || parseFloat(createLongitude) <=
        -180) {
      NotificationManager.error('Enter a valid longitude value.', 'Error');
      return;
    }
    try {
      const request = await fetch(host + `api/admin/Depos`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
                  ...
                  {depots.map((depot, idx) => (
                    <tr key={idx}>
                      ...
                      <td>
                        <div className="sameLine">
                          ...
                          <Form.Control type="text"
                            onInput={(e) => setNewLatitude(
                                e.target.value)}
                            size={12}/>
                          <div className="buttonPadding">
                            <Button onClick={() => {
                              if (parseFloat(depot.latitude) !==
                                  parseFloat(newLatitude)) {
                                editDepot(depot.depoId, 1);
                              } else {
                                NotificationManager.error(
                                    'Latitude cannot be the same.', 'Error');
                              }
                            }}>
                              Edit
                            </Button>
                      ...
                      <td>
                        <div className="sameLine">
                          <div
                            className="maxWidthDepot"> {depot.longitude} </div>
                          <Form.Control type="text"
                            onInput={(e) => setNewLongitude(
                                e.target.value)}
                            size={12}/>
                          <div className="buttonPadding">
                            <Button onClick={() => {
                              if (parseFloat(depot.longitude) !==
                                  parseFloat(newLongitude)) {
                                editDepot(depot.depoId, 2);
                              } else {
                                NotificationManager.error(
                                    'Longitude cannot be the same.', 'Error');
                              }
                            }}>
                              Edit
                            </Button>
                      ...
                      <td>
                        <Button onClick={() => deleteDepot(depot.depoId)}
                          variant="danger">
                          Delete
                        </Button>
                      </td>
